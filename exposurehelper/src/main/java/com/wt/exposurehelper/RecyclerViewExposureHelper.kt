...
import com.wt.exposurehelper.model.InExposureData
import com.wt.exposurehelper.model.VisibleItemPositionRange
...
//        Log.d(this.logTag, "当前可见的position范围: $visiblePositions")
            ...
            visiblePositionBindExposureDataList.forEach { visiblePositionBindExposureData ->
                if (visiblePositionBindExposureData !in inExposureDataList) {
                    ...
                    invokeExposureStateChange(
                        visiblePositionBindExposureData,
                        visiblePosition,
                        true
                    )
                }
        ...
        inExposureDataList.filter { inExposureData ->
            ...
            it.forEach { inExposureData ->
                invokeExposureStateChange(inExposureData, inExposureData.position, false)
            }
            //移除出正在曝光中的集合
...
//            Log.w(this.logTag, "position为${position}的ItemView没有实现IProvideExposureData接口,无法处理曝光")
    ...
    //列表删除某个item时调用  结束曝光
    fun removeExposureData(position: Int, bindData: BindExposureData?) {
        for (i in inExposureDataList.count() - 1 downTo 0) {
            val inExposureData = inExposureDataList[i]
            if (bindData == inExposureData.data) {
                invokeExposureStateChange(inExposureData, position, false)

                //移除出正在曝光中的集合
                inExposureDataList.remove(inExposureData)
                return
            }
        }
    }
    ...
    private fun invokeExposureStateChange(
        inExposureData: InExposureData<BindExposureData>,
        position: Int,
        inExposure: Boolean
    ) {
        try {
            exposureStateChangeListener.onExposureStateChange(
                ...
                duration = System.currentTimeMillis() - inExposureData.startTime
            )
